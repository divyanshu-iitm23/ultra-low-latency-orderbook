// udp_publish_demo.cpp - publish aggregator snapshots as JSON UDP datagrams
//   ./udp_publish_demo [seconds] [host] [port]      (defaults: 8s 127.0.0.1 9099)
//
...
int main(int argc, char** argv) {
    const double  seconds = (argc > 1) ? atof(argv[1]) : 8.0;
    const char*   host    = (argc > 2) ? argv[2] : "127.0.0.1";
    const uint16_t port   = (argc > 3) ? (uint16_t)atoi(argv[3]) : 9099;
    std::signal(SIGINT, on_sigint);

    UdpPublisher pub(host, port);
    if (!pub.ok()) { fprintf(stderr, "udp_publish_demo: bad host/port %s:%u\n", host, port); return 1; }
    fprintf(stderr, "udp_publish_demo: publishing snapshots to %s:%u for %.0fs\n", host, port, seconds);
    ...
    fprintf(stderr, "udp_publish_demo: done  datagrams sent=%llu errors=%llu  (events=%llu drops=%llu)\n",
