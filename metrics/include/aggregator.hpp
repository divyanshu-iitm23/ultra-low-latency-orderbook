...
private:
    ...
    void render(uint64_t t_now, uint64_t t_start, bool final) {
        ...
        for (uint32_t op = 0; op < NUM_OPS; ++op) {
            ...
            fmt(win_[op].percentile(50.0),    p50,  sizeof p50);
            fmt(win_[op].percentile(99.0),    p99,  sizeof p99);
            fmt(win_[op].percentile(99.9),    p999, sizeof p999);
            fmt(cum_[op].max(),               mx,   sizeof mx);   // sticky worst case
